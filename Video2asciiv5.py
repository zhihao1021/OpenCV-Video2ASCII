...
print('進行轉檔......')
os.system(f'{ffmpeg_path}ffmpeg -v error -i "{media_list[number]}" -c:v h264 -s {int(height * (90 / width))}x90 -y temp.mp4')
...
def load():
    ...
    for con in range(int(frames)):
        ...
        temp = '\n' + str(temp.tolist()).replace('\'], [\'', '\n').replace('\', \'', '')
        out_list.append(temp[:-3])
...
def play_video():
    ...
    for content in out_list:
        ...
        sys.stdout.write(f'{content}\n{time_1[0]}:{time_1[1]}[{"=" * proc[0]}{"-" * proc[1]}{" " * proc[2]}]{time_2[0]}:{time_2[1]}\nP/C/A:{played}/{len(out_list)}/{frames} T:{format(time.time() - st_time, ".2f")}s FPS(avg./tgt.):{now_fps}/{fps} SleepTime:{format(sleep_time / 10, ".1f")}ms')
